...
fun FollowingsScreen(
    ...
    val derivedState = remember {
        derivedStateOf {
            when (state) {
                FollowingsViewModel.State.Empty -> DesignStreamState.Default
                FollowingsViewModel.State.Loading -> DesignStreamState.Loading
                is FollowingsViewModel.State.Success -> {
                    DesignStreamState.Success(
                        (state as FollowingsViewModel.State.Success).content
                    )
                }
                is FollowingsViewModel.State.Error -> DesignStreamState.Error(
    ...
    LaunchedEffect(Unit) {
        if (state is  FollowingsViewModel.State.Empty) {
