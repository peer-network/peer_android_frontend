...
internal class FollowersViewModelTest {
    @get:Rule
    val instantExecutorRule = InstantTaskExecutorRule()

    private val dispatcher = UnconfinedTestDispatcher()

    private val usecase = mockk<FollowerPagingUsecase>()
    ...
    fun setup() {
        ...
        viewModel = FollowersViewModel(usecase)
    }
    ...
    fun `test follow member`() = runTest {
        ...
        viewModel.state.test {
            assertTrue(awaitItem() is FollowersViewModel.State.Loading)
            assertTrue(awaitItem() is FollowersViewModel.State.Success)
        }
    }
    ...
    fun `test follow member error`() = runTest {
        ...
        viewModel.followers(id, Pageable(0, 1))
        viewModel.state.test {
            assertEquals(FollowersViewModel.State.Error(error), awaitItem())
        }
    }
}
