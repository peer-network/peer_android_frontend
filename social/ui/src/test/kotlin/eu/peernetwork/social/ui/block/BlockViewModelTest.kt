...
internal class BlockViewModelTest {
    ...
    fun `test block list`() = runTest {
        ...
        viewModel.state.test {
            assertTrue(awaitItem() is BlockViewModel.State.Loading)
            assertTrue(awaitItem() is BlockViewModel.State.Success)
        }
    }
    ...
    fun `test block list error`() = runTest {
        ...
        viewModel.blockList(id, Pageable(0,1))
        viewModel.state.test {
            assertEquals(BlockViewModel.State.Error(error), awaitItem())
        }
    }
}